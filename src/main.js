...
function renderNav() {
  const groups = [
    ...
    { title: '进阶 · 视野', ids: ['ch85', 'ch12', 'hard', 'rag', 'prompt', 'sec'] },
...
function chapterView(c) {
  ...
  if (c.cards) {
    ...
    else if (c.id === 'hard') { cardTitle = '大厂 Agent 硬核 12 问'; cardHint = '点右上角「揭」看深度答案 + 面试官考察意图，先看题自己答'; }
    else if (c.id === 'rag') { cardTitle = 'RAG 深度 6 问'; cardHint = '点右上角「揭」看深度答案 + 面试官考察意图，先看题自己答'; }
